...
You can read more about the manifest file [here](https://support.zendesk.com/hc/en-us/articles/4408846524954-Customizing-the-Settings-panel-of-the-theme).
...
The theme has three separate places where translated strings live, each with its own mechanism:

## Theme settings (`translations.yml`)

The top-level `translations.yml` file is reserved for strings referenced by the `manifest.json` settings (labels, descriptions and list options shown in the Theming Center Settings panel). See [Customizing the Settings panel of the theme](https://support.zendesk.com/hc/en-us/articles/4408846524954-Customizing-the-Settings-panel-of-the-theme#translations) for details. Do not add strings used by templates or React components to this file.

The locale JSON files under `translations/` are generated by `yarn download-locales`, which fetches the translated versions of `translations.yml` from Zendesk's translation system. This script is only useful for the official Zendesk-maintained theme; custom forks should manage their locale JSON files manually.

## Curlybars templates

Templates under `templates/` can localize strings using the [`{{t}}` translation helper](https://developer.zendesk.com/api-reference/help_center/help-center-templates/helpers/#t-translation-helper). The helper only resolves keys that Help Center exposes to the theme, so a new key cannot be used in a template until it has been added to the set of keys available to the `t` helper. Strings added only to `translations.yml` or to a React module's `translations/` folder are not reachable from `{{t}}`.

## React components

React components use `react-i18next` with YAML translation files under `src/modules/[MODULE]/translations/`. See [Internationalization](#internationalization) in the React components section for the full workflow (adding strings, plurals, extraction and updating translation files). The `yarn i18n:update-translations` script that downloads the latest locale files is, like `yarn download-locales`, only useful for the official Zendesk-maintained theme.
